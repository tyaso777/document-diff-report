...
対応形式: **PDF / docx / pptx / xlsx / xlsm / csv / txt / html / htm**(新旧は同じ形式同士で比較してください)

## レポートの機能

- 旧版(左)/新版(右)の2カラム。対応する段落は常に同じ行に横並び
- 変更行は黄色、追加は緑、削除は赤。段落内の変更は文字単位で `del`/`ins` ハイライト
- 各段落に元文書内の位置ラベル(PDF: p.3、pptx: s.2、xlsx: Sheet!7、csv:7 など)
- ヘッダに変更/追加/削除の件数サマリ
- 「変更箇所のみ表示」トグル
...
PDFを比較しない場合(docx/pptx/xlsx/csv/txt/htmlのみ)、pdfiumライブラリは不要です。
...
- **xlsx / xlsm / csv** — 「1行=1ブロック」で直列化します(例: `B: 提出日 ┃ D: 入力必須…`)。
  ...
  ※ .csv はヘッダー行も含め、全行を比較対象にします。
  ※ .csv の文字コードは UTF-8 / UTF-8 BOM付き / CP932(Shift_JIS) に対応します。
- **txt** — 空行や見出しらしい行を境界として段落ブロック化します。文字コードは UTF-8 / UTF-8 BOM付き / CP932(Shift_JIS) に対応します。
- **html / htm** — `script` / `style` / `noscript` を除外し、見出し・段落・リスト・表などの主要タグを段落境界として本文テキスト化します。文字コードは UTF-8 / UTF-8 BOM付き / CP932(Shift_JIS) に対応します。
